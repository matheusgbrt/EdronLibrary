...
import { ExtraSlotItem, Vocation } from '../validation/tibia-item.schema.js';
...
const LIGHT_RADIUS_KEYS = ['lightradius', 'light radius'];
const LIGHT_COLOR_KEYS = ['lightcolor', 'light color'];
const ATTACK_KEYS = ['attack', 'atk', 'attackvalue'];
const CHARGE_KEYS = ['charges'];
const DURATION_KEYS = ['duration'];
...
function parseDurationSeconds(value: string | undefined): number | null {
  ...
  const normalized = stripWikiMarkup(value).toLowerCase();
  const number = parseInteger(normalized);
  if (number === null) {
    return null;
  }
...
export function normalizeExtraSlotItem(
  raw: RawCapturedItem,
  ...
  if (!subtype) {
    throw new Error(`Unsupported extra-slot category: ${category.key}`);
  }
  ...
  const lightRadius = parseInteger(getField(raw.rawFields, LIGHT_RADIUS_KEYS));
  ...
  return {
    ...
    metadata: {
      tags: ['extra-slot', category.key, subtype.toLowerCase(), ...id.split('-')],
      dataQuality: imagePath ? 'complete' : 'partial',
    },
    extraSlot: {
      subtype,
      providesLight: lightRadius !== null && lightRadius > 0,
      lightColor: getField(raw.rawFields, LIGHT_COLOR_KEYS) ?? null,
      lightRadius,
      attack: parseInteger(getField(raw.rawFields, ATTACK_KEYS)),
      charges: parseInteger(getField(raw.rawFields, CHARGE_KEYS)),
      durationSeconds: parseDurationSeconds(getField(raw.rawFields, DURATION_KEYS)),
      consumable: false,
    },
  };
}
